...
const DetailTafsir = ({route}) => {
  ...
  useEffect(() => {
    const findDetail = () => {
      try {
        fetch(`https://equran.id/api/v2/tafsir/${route.params.id}`)
          .then(res => res.json())
          .then(json => {
            setDatas(json.data);
            setIsLoading(false);
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView
        ...
        <TouchableOpacity className="flex items-center justify-center bg-[#863ED5] object-cover  w-full h-[265px] rounded-[20px] py-7 shadow-lg">
          <Image
            source={Quran}
            alt="belum"
            style={{
        ...
        <View className="flex-row w-full justify-between bg-[#12193112] rounded-[10px] py-[10px] px-[13px]">
          <View className="bg-[#863ED5]items-center justify-center rounded-full">
            <Text className="text-[#863ED5] font-medium">
              Tafsir Surah {datas.namaLatin}
            </Text>
          </View>
          <View>
            <TouchableOpacity
        ...
        <View>
          {(datas.tafsir || []).map((item, i) => (
            <View key={i} className="bg-white w-full py-2 px-2">
              <View className="mt-6">
                <View className="flex justify-end">
                  <Text className="font-bold text-xl">{item.ayat}</Text>
                </View>
                <View className="flex justify-start mt-4">
                  <Text className="font-medium text-md">{item.teks}</Text>
                </View>
              </View>
